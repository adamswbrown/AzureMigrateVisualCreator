...
Function select-subscription {activesubs | Out-GridView -PassThru | Select-AzSubscription}


Function Get-ActiveResoruceGroups { Get-AzResourceGroup | Where-Object -Property ProvisioningState -eq "Succeeded" }
Function select-resorucegroup {Get-AzResourceGroup | Out-GridView -PassThru  }
...
3. Handle caes of 1 discovered machine.
#>
function Get-AzureMigrateDiscoveredSQLMachine {
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory = $true)][string]$Token,
        [Parameter(Mandatory = $true)][string]$SubscriptionID,
    ...
    if($GroupName) {
        $url = "https://management.azure.com/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Migrate/assessmentprojects/{2}/machines?api-version=2019-05-01&pageSize=100&%24filter=(Properties/SqlInstancesCount%20gt%200)"  -f $SubscriptionID, $ResourceGroup, $Project, $GroupName
    }
...
https://management.azure.com/subscriptions/0746b654-25c4-4649-add9-75391436e3dd/resourceGroups/Migration-Assessment/providers/Microsoft.Migrate/assessmentprojects/AzureMigrate4007project/machines?api-version=2019-05-01&pageSize=10000&%24filter=(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMigrate4007site%27))%20or%20(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMirgate5037site%27))&includeOnlyIds=true
#>
function Get-AzureMigrateDiscoveredAVSMachine {
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory = $true)][string]$Token,
        [Parameter(Mandatory = $true)][string]$SubscriptionID,
    ...
    $obj = @()
    $url = "https://management.azure.com/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Migrate/assessmentProjects/{2}/machines?api-version=2019-05-01&pageSize=10000&%24filter=(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMigrate4007site%27))%20or%20(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMirgate5037site%27))&includeOnlyIds=true" -f $SubscriptionID, $ResourceGroup, $Project
    if($GroupName) {
        $url = "https://management.azure.com/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Migrate/assessmentprojects/{2}/machines?api-version=2019-05-01&pageSize=10000&%24filter=filter=(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMigrate4007site%27))%20or%20(contains(Properties/DiscoveryMachineArmId%2C%27VMwareSites/AzureMirgate5037site%27))&includeOnlyIds=true"  -f $SubscriptionID, $ResourceGroup, $Project, $GroupName
    }
...
<#
.SYNOPSIS
Create a new VM assessment for an existing Azure Migrate group.
.DESCRIPTION
...
<#
.SYNOPSIS
Create a new SQL assessment for an existing Azure Migrate group.
.DESCRIPTION
The New-AzureMigrateAssessment cmdlet creates a new assessment of an existing Azure Migrate group using the parameters supplied.
.PARAMETER Token
Specifies an authentication token to use when retrieving information from Azure.
...
.EXAMPLE
Create a new assessment named "Assessment01"
PS C:\>New-AzureMigrateSQLAssessment -Token $token -SubscriptionID XX -ResourceGroup YY -Project ZZ -AssessmentName "Assessment01" -AssessmentProperties .\SampleAssessmentProperties01.json
...
<#
.SYNOPSIS
Downloads Software Inventory
.DESCRIPTION
The Export-SoftwareInvetory function Exports the VMware Software Inventory
.PARAMETER Token
Specifies an authentication token to use when retrieving information from Azure.
.PARAMETER SubscriptionID
The subscription the project resources are contained in.
.PARAMETER VMwareSite
The Name of the VMwareSite to referance.

.EXAMPLE
Export Software Inventory for Site 01 
PS C:\>Export-SoftwareInventory -Token $token -SubscriptionID XX -ResourceGroup YY -Project ZZ - -VMwareSite 01
...
function Export-SoftwareInventory {
    ...
    #$obj = @()
    $url = "https://management.azure.com/subscriptions/"+$subscriptionId+"/resourceGroups/"+$ResoruceGroup+"/providers/Microsoft.OffAzure/VMwareSites/"+$VmwareSite+"/ExportApplications?api-version=2018-05-01-preview"
